...
# 수정탑 건설
    async def build_pylons(self):
        if self.supply_left < 8 and not self.already_pending(PYLON): # 인구수가 8보다 적게남고(OK), 수정탑 건설 진행중인 수정탑이 없으면
            ...
            # elif not nexuses.exists: # 넥서스가 없으면 게이트웨이 주변에 수정탑 생성
            #     if self.can_afford(PYLON):
            #         gt = self.units(GATEWAY).ready
            #         await self.build(PYLON, near=gt)
...
# 게이트웨이, 인공제어소, 스타게이트, 제련소, 캐논 생성
    async def offensive_force_buildings(self):
        ...
        if self.units(PYLON).ready.exists:
            ...
            elif len(self.units(FORGE)) < 1:
                if self.can_afford(FORGE) and not self.already_pending(FORGE):
                    await self.build(FORGE, near=pylon)

            if self.units(FORGE).ready.exists and len(self.units(PHOTONCANNON)) < ((self.iterartion / self.ITERATION_PER_MINUTE) /2 ):
                if self.can_afford(PHOTONCANNON):
                    if self.units(VOIDRAY).amount > 6:
                        await self.build(PHOTONCANNON, near=nx)
